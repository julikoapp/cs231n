...
def adam(w, dw, config=None):
    ...
    config.setdefault("epsilon", 1e-8)
    config.setdefault("m", np.zeros_like(w)) # is just like scalar
    ...
    # m is the first moment as mean of the gradient
    # v is the second moment as uncentered variance of the gradient

    m = config["beta1"]* config["m"] + (1 - config["beta1"]) * dw
    v = config["beta2"]* config["v"] + (1 - config["beta2"]) * (dw**2)
    config["m"] = m #. shoudl update before!
    config["v"] = v
    config["t"] = config["t"]+1
    m = m / (1- np.power(config["beta1"], config["t"]))
    v = v / (1 - np.power(config["beta2"], config["t"]))
