...
class ThreeLayerConvNet(object):
    ...
    def loss(self, X, y=None):
        ...
        # x1, conv_relu_pool_cache = conv_relu_pool_forward(X, W1, b1,conv_param ,pool_param)
        # x2, affine_relu_cache = affine_relu_forward(x1, W2, b2)
        # x3, affine2_cache = affine_forward(x2, W3, b3)
        ...
        # loss, dx = softmax_loss(x3, y) #  return loss, dx
        # for layeri in range(1, 4):
        #   loss +=  0.5* self.reg * (np.sum(self.params['W'+str(layeri)]**2))
        # # print("WORKS fine 1")
        # dx1, dw1, db1 = affine_backward(dx, affine2_cache)
        # grads["W3"], grads["b3"] = dw1+self.reg* self.params['W3'], db1
        # dx2, dw2, db2 = affine_relu_backward(dx1, affine_relu_cache) #return dx, dw, db
        # grads["W2"], grads["b2"] = dw2+self.reg* self.params['W2'], db2
        # dx3, dw3, db3 = conv_relu_pool_backward(dx2, conv_relu_pool_cache) #return dx, dw, db
        # grads["W1"], grads["b1"] = dw3+self.reg* self.params['W1'], db3
