...
class KNearestNeighbor(object):
    ...
    def compute_distances_no_loops(self, X):
        ...
        dists = (-2*np.dot(X, self.X_train.T) + sx + sy.T)
    ...
    def predict_labels(self, dists, k=1):
        ...
        for i in range(num_test):
            ...
            for j in range(k):
              closest_y.append(self.y_train[sorted_ind[j]])
            # for i in range(3):
            #   self
            # *****END OF YOUR CODE (DO NOT DELETE/MODIFY THIS LINE)*****
            #########################################################################
            ...
            #########################################################################
            # *****START OF YOUR CODE (DO NOT DELETE/MODIFY THIS LINE)*****
            count_arr = [ closest_y.count(x) for x in closest_y]
            max_cnt = max(count_arr)
            possible_y = []
            for j in range(len(count_arr)):
              if count_arr[j] == max_cnt:
                possible_y.append(closest_y[j])
            
            y_pred[i] = min(possible_y)
